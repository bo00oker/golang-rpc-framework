...
import (
	...
	"github.com/rpc-framework/core/internal/user/handler"
...
func main() {
	...
	userSvc := service.NewUserService(userRepo, tracer)
	userHandler := handler.NewUserHandler(userSvc, tracer)

	// 注册服务
	srv.RegisterService(&user.UserService_ServiceDesc, userHandler)
	...
	log.Infof("User service started successfully on %s", serverCfg.Address)
